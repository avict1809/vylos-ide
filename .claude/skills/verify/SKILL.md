...
- Real mouse/keyboard on the explorer or editor works via `Input.dispatchMouseEvent` / `Input.dispatchKeyEvent` (`rawKeyDown` + `keyUp`, `modifiers: 2` = Ctrl) and `Input.insertText` for typing. When looking up a file row by name, scope to `.custom-scrollbar` — the title-bar breadcrumb has a span with the same text.
- Main-process IPC (`electron/main.ts`) can be tested without launching Electron: `npx tsc -p electron/tsconfig.json --outDir <scratchpad>/edist`, then in a Node script hook `Module._load` to stub `electron`, `electron-serve`, `electron-updater` (record `ipcMain.handle` and `webContents.on` callbacks), require the compiled `main.js`, and call handlers on a temp folder. Run with `NODE_PATH=<repo>/node_modules`.
